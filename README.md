...
### 2.1 后台文件上传（简历 / 二维码 / 封面）前置条件

后台「上传文件」按钮会优先把文件存到 **CloudBase 云存储**（私有存储，不开公开读）；数据库里存的是 `cloud://` 文件 ID，构建/部署时由站点统一签发 **1 年有效**的签名访问 URL（文件始终不公开）。若云存储不可用则自动回退到本地 `public/uploads/`。要让云存储生效，需满足：

1. 在 CloudBase 控制台**开通云存储**（与文档库同一环境）；
2. **无需设为公开读**：文件保持私有，靠签名 URL 访问（避免公开读权限及其成本；签发的 URL 默认 1 年有效，每次重新部署/构建会重新签发，长期可用）；
3. 已正确配置 `CLOUDBASE_ENV_ID / CLOUDBASE_SECRET_ID / CLOUDBASE_SECRET_KEY`（同拉取内容的三个变量）。

> 若未开通云存储 / 上传失败：上传按钮仍可用，文件会落到本地 `public/uploads/`（路径进入静态构建产物），功能不中断，只是文件不在云端。
